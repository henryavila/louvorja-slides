...
class DocumentTest(unittest.TestCase):
    ...
    def test_single_word_transcript_does_not_crash_gap_grouping(self) -> None:
        transcript = Transcript(
            words=[TranscribedWord("Santo", 1.0, 1.4)],
            detected_language="pt",
            duration_seconds=10.0,
        )

        doc = transcript_to_document(transcript, title="Uma palavra")
